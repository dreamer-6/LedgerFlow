...
import { getUserFromToken, resolveCompanyId } from '../api/routes.js';

export class MasterController {
  constructor(private db: DatabaseSync) {}

  getLedgers = (req: Request, res: Response) => {
    try {
  ...
  createLedger = (req: Request, res: Response) => {
    try {
      const user = getUserFromToken(req);
  ...
  getParties = (req: Request, res: Response) => {
    try {
      const user = getUserFromToken(req);
      const companyId = resolveCompanyId(req, this.db, user);
      if (!companyId) return res.json([]);
  ...
  createParty = (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const group = this.db.prepare('SELECT group_id FROM ledger_groups WHERE company_id = ? AND code = ?').get(targetCompanyId, groupCode) as any;
  ...
  updateParty = (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const party = this.db.prepare('SELECT ledger_id FROM parties WHERE party_id = ?').get(id) as any;
  ...
  deleteParty = (req: Request, res: Response) => {
    try {
      ...
      const usage = (this.db.prepare('SELECT COUNT(*) as count FROM vouchers WHERE party_id = ?').get(id) as any)?.count || 0;
      if (usage > 0) return res.status(400).json({ error: 'Cannot delete party as it has associated vouchers.' });
  ...
  createItem = (req: Request, res: Response) => {
    try {
      ...
      if (!godownId || godownId === 'godown_main') {
        const defGodown = this.db.prepare('SELECT godown_id FROM godowns WHERE company_id = ? LIMIT 1').get(targetCompanyId) as any;
      ...
      if (existing) {
        ...
        this.db.prepare(`
          ...
          WHERE item_id = ?
        `).run(
          b.hsnSac || null, b.gstRate || null, rate, rate, sellRate, sellRate,
          b.reorderLevel || null, updatedSerials || null, b.hasSerialNo ? 1 : 0,
          b.serialNumbers || null, existing.item_id
        );
      ...
      if (!unitId || unitId === 'unit_nos') {
        const defUnit = this.db.prepare('SELECT unit_id FROM units WHERE company_id = ? LIMIT 1').get(targetCompanyId) as any;
      ...
      `).run(
        ...
        unitId, b.gstRate || 18, b.cessRate || 0, rate, sellRate,
        qty, rate, b.reorderLevel || 0, b.serialNumbers || null, b.hasSerialNo ? 1 : 0
      );
  ...
  updateItem = (req: Request, res: Response) => {
    try {
      ...
      `).run(
        ...
        b.reorderLevel || 0, req.params.id
      );
  ...
  deleteItem = (req: Request, res: Response) => {
    try {
      ...
      const lineCount = (this.db.prepare('SELECT COUNT(*) as cnt FROM voucher_lines WHERE item_id = ?').get(itemId) as any)?.cnt || 0;
      if (lineCount > 0) {
  ...
  getGodowns = (req: Request, res: Response) => {
    try {
      const user = getUserFromToken(req);
      const companyId = resolveCompanyId(req, this.db, user);
      if (!companyId) return res.json([]);
